...
@xport/uni-cn can be used in two ways:
...
|-------------|-----------|-------------------|-------------|
| **Node.js** | Real disk (`NodeFileSystem`) | `npm install` via execa | `@xport/uni-cn` |
| **Browser** | In-memory (`MemFileSystem` / memfs `Volume`) | Writes `package.json` directly (no npm) | `@xport/uni-cn/browser` |
...
npx @xport/uni-cn init

# Initialize with defaults, skip prompts
npx @xport/uni-cn init -y -d

# Add components
npx @xport/uni-cn add button -y
npx @xport/uni-cn add button card dialog -y

# Add all available components
npx @xport/uni-cn add -a -y

# Add a template (scaffold files as-is from registry)
npx @xport/uni-cn add template default --name my-project --style default
```
...
```bash
REGISTRY_URL=https://ui.shadcn.com/r npx @xport/uni-cn init -y -d
```
...
Import from `@xport/uni-cn`. Operations use real filesystem and can run `npm install`.

```ts
import { runInit } from '@xport/uni-cn';
...
import { initServiceModules } from '@xport/uni-cn';
import { InitCommandServiceId } from '@xport/uni-cn';
...
Import from `@xport/uni-cn/browser`. All I/O goes through a memfs `Volume` — no real filesystem or shell is involved.
...
#### `@xport/uni-cn/browser` (Browser / Memfs)
...
- `NPM_TOKEN`: an npm token allowed to publish `@xport/uni-cn`.
...
For the `@xport` scope, make sure the token owner has publish access to that npm user or organization scope.
