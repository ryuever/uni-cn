...
export const TransformTwPrefixServiceId = createId(
  'transform-tw-prefix-service-id'
);
@injectable()
export class TransformTwPrefixService {
  constructor(
  ...
  async transformTwPrefix(opts: TransformOpts): Promise<CodemodPlugin> {
    ...
    return {
      ...
      transform({
        scriptASTs,
        sfcAST,
        utils: { traverseScriptAST, traverseTemplateAST, astHelpers },
      }) {
        ...
        function isVariantProperty(node: any): boolean {
          if (node.type === 'Property') {
            ...
            if (node.key?.type === 'Identifier') {
              ...
              return ['variant', 'size', 'color', 'type', 'state'].includes(
                keyName
              );
            }
            if (
            ...
            expression.arguments.forEach((arg: any) => {
              if (arg.type === 'Literal' && typeof arg.value === 'string') {
                ...
                if (
                  ...
                  arg.consequent.value = applyPrefix(
                ...
                if (
                  ...
                  arg.alternate.value = applyPrefix(
                    arg.alternate.value,
                    config.tailwind.prefix,
                    tailwindVersion
                  );
              ...
              if (
                ...
                if (args[1]?.type === 'ObjectExpression') {
                  ...
                  if (
                    ...
                    variantsProperty.value.type === 'ObjectExpression'
                  ) {
                    // Only transform class strings in variants, not the variant names themselves
                    const allProperties = astHelpers.findAll(
              ...
              if (
                ...
                path.node.arguments.forEach((arg) => {
                  if (arg.type === 'Literal' && typeof arg.value === 'string') {
                    ...
                    const literals = astHelpers.findAll(arg, {
                      type: 'Literal',
                    });
            ...
            enterNode(node) {
              ...
              if (
                ...
                if (node.key.argument?.type === 'VIdentifier') {
                  ...
                  if (
                    ['class', 'className', 'classes', 'classNames'].includes(
                      argName
                    )
                  ) {
              ...
              // Handle static class attributes
                ...
                if (
                  ...
                  const prefixedValue = applyPrefix(
                    cleanValue,
                    config.tailwind.prefix,
                    tailwindVersion
                  );
