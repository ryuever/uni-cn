...
export const TransformCssVarsServiceId = createId(
  'transform-css-vars-service-id'
);
@injectable()
export class TransformCssVarsService {
  constructor(
  ...
  async transformCssVars(
    ...
    if (options.tailwindVersion === 'v4') {
      ...
      if (config.resolvedPaths?.cwd) {
        const packageInfo = await this.getPackageInfoService.getPackageInfo(
      ...
      if (options.tailwindConfig) {
        ...
        plugins.push(
          updateTailwindConfigAnimationPlugin(options.tailwindConfig)
        );
        plugins.push(
          updateTailwindConfigKeyframesPlugin(options.tailwindConfig)
        );
      }
    }
...
export const UpdateCssVarsServiceId = createId('update-css-vars-service-id');
@injectable()
export class UpdateCssVarsService {
  constructor(
    @inject(FileSystemServiceId)
    private readonly fileSystemService: IFileSystemService,
    @inject(TransformCssVarsServiceId)
    private readonly TransformCssVarsService: TransformCssVarsService
  ) {}
  ...
  async updateCssVars(
    ...
    const output = await this.TransformCssVarsService.transformCssVars(
      raw,
      cssVars ?? {},
      config,
      {
