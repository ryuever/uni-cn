...
export const UpdateFilesServiceId = createId('update-files-service-id');
export const ResolveImportsServiceId = createId('resolve-imports-service-id');
    ...
    for (const file of files) {
      ...
      if (existingFile) {
        const existingFileContent =
          await this.fileSystemService.promisifyFs.readFile(filePath, 'utf-8');
...
@injectable()
export class ResolveImportsService {
  constructor(
    @inject(FileSystemServiceId)
    private readonly fileSystemService: IFileSystemService,
    @inject(GetProjectInfoServiceId)
  ...
  async resolveImports(filePaths: string[], config: Config) {
    const projectInfo = await this.getProjectInfoService.getProjectInfo(
      config.resolvedPaths.cwd
    );
    const tsConfig = await getTSConfig(
      config.resolvedPaths.cwd,
      projectInfo?.typescript ? 'tsconfig.json' : 'jsconfig.json'
    );
    const updatedFiles = [];
    ...
    for (const filepath of filePaths) {
      ...
      const content = await this.fileSystemService.promisifyFs.readFile(
        resolvedPath,
        'utf-8'
      );
        ...
        const importResolverTransformer = (opts: any) => ({
          ...
          transform(node: any) {
            ...
            if (node.type === 'ImportDeclaration' && node.source?.value) {
              ...
              const probableImportFilePath = resolveImportSync(
                moduleSpecifier,
                tsConfig
              );
