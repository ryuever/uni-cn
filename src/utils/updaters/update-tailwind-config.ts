...
import {
  Project, QuoteKind, ScriptKind, SyntaxKind,
} from 'ts-morph'
...
export const UpdateTailwindConfigServiceId = createId(
  'update-tailwind-config-service-id',
)
export const TransformTailwindConfigServiceId = createId(
  'transform-tailwind-config-service-id',
)
export const CreateSourceFileServiceId = createId(
  'create-source-file-service-id',
)
export const AddTailwindConfigThemeServiceId = createId(
  'add-tailwind-config-theme-service-id',
)
export const ParseObjectLiteralServiceId = createId(
  'parse-object-literal-service-id',
)
...
// 2. ParseObjectLiteralService - depends on CreateSourceFileService
@injectable()
export class ParseObjectLiteralService {
  constructor(
    @inject(CreateSourceFileServiceId)
    private readonly createSourceFileService: CreateSourceFileService,
  ) {}

  async parseObjectLiteral(objectLiteralString: string): Promise<any> {
...
// 3. AddTailwindConfigThemeService - depends on ParseObjectLiteralService
@injectable()
export class AddTailwindConfigThemeService {
  constructor(
    @inject(ParseObjectLiteralServiceId)
    private readonly parseObjectLiteralService: ParseObjectLiteralService,
  ) {}

  async addTailwindConfigTheme(
...
// 4. TransformTailwindConfigService - depends on CreateSourceFileService and AddTailwindConfigThemeService
@injectable()
export class TransformTailwindConfigService {
  constructor(
    @inject(CreateSourceFileServiceId)
    private readonly createSourceFileService: CreateSourceFileService,
    @inject(AddTailwindConfigThemeServiceId)
    private readonly addTailwindConfigThemeService: AddTailwindConfigThemeService,
  ) {}

  async transformTailwindConfig(
    input: string,
    ...
    const configObject = sourceFile
      ...
      .find(node => node
        .getProperties()
        .some(
          property => property.isKind(SyntaxKind.PropertyAssignment)
    ...
    if (tailwindConfig.theme) {
      await this.addTailwindConfigThemeService.addTailwindConfigTheme(
        configObject,
...
// 5. UpdateTailwindConfigService - depends on TransformTailwindConfigService
@injectable()
export class UpdateTailwindConfigService {
  constructor(
    @inject(FileSystemServiceId)
    private readonly fileSystemService: IFileSystemService,
    @inject(TransformTailwindConfigServiceId)
    private readonly transformTailwindConfigService: TransformTailwindConfigService,
  ) {}
  ...
  async updateTailwindConfig(
    ...
    const output = await this.transformTailwindConfigService.transformTailwindConfig(
      raw,
      tailwindConfig,
      config,
    )
